...
exports.createStudentValidator =[
    ...
    .custom((val)=> student.findOne({email : val}).then((user) =>{
        if(student){
    ...
    check('password')
      ...
      .custom(async (val , {req}) => {
        try {
          ...
          const isCorrect = await bcrypt.compare(req.body.currentPassword, studentInstance.password);
          if (!isCorrect) {
    ...
    check('password')
      ...
      .custom(async (val , {req}) => {
        try {
          ...
          const isCorrect = await bcrypt.compare(req.body.currentPassword, studenttInstance.password);
          if (!isCorrect) {
            throw new Error('Incorrect Current Password');
          }
